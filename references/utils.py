...
class ResNet(nn.Module):
    def __init__(self, depth, out_indices, num_classes, **kwargs):
        super().__init__()
        expansion = 4 if depth > 34 else 1
        self.resnet = MODELS.build(dict(
            type='mmdet.ResNet',
            depth=depth,
            out_indices=out_indices,
            conv_cfg=dict(type='Conv2d', spectral_norm=True),
            **kwargs
        ))
    ...
    def forward(self, x):
        return self.final(self.resnet(x)[-1])

def mmdetection_get_model(model_name, num_classes, weights = None, progress = True, **config: Any) -> nn.Module:
    """
    Gets the model name and configuration and returns an instantiated model.

    Args:
        model_name (str): The name under which the model is registered.
        num_classes (int): The number of classes for the classification head.
        weights (Optional[str]): The weights to load. If None, no weights are loaded.
        **config (Any): parameters passed to the model builder method.

    Returns:
        model (nn.Module): The initialized model.
    """
    # Remove prepended "mmdetection-" from the model name
